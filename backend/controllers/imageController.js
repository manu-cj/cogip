...
const saveUserImage = (req, res) => {
  ...
  uploadUserImage.single("image")(req, res, async (err) => {
    ...
    const imageInfo = {
      filename: req.file.filename,
      path: `public/assets/img/people/users/${req.file.filename}`,
      originalName: req.file.originalname,
      uploadDate: new Date(),
    };
    try {
      ...
      if (!editedUser) {
        return res.status(404).json({
          message:
            "Could not update contact image. Make sure the user Id is valid.",
        });
      }
      return res.status(200).json({
...
export { saveContactImage, saveUserImage };
